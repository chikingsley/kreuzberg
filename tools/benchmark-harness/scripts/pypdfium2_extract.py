"""pypdfium2 extraction wrapper for benchmark harness.
...
def extract_sync(file_path: str) -> dict[str, Any]:
    ...
    return {
        "content": markdown,
        "metadata": {"framework": "pypdfium2"},
        "_extraction_time_ms": duration_ms,
    }


def extract_batch(file_paths: list[str]) -> list[dict[str, Any]]:
    """Extract multiple files (simulated batch - pypdfium2 has no native batch API)."""
    start = time.perf_counter()
    results = []

    for file_path in file_paths:
        try:
            results.append(extract_sync(file_path))
...
def main() -> None:
    ...
    for arg in sys.argv[1:]:
        if arg in ("--ocr", "--no-ocr"):
            pass  # Accepted but ignored - pypdfium2 doesn't provide OCR
    ...
    if len(args) < 1:
        print(
            "Usage: pypdfium2_extract.py [--ocr|--no-ocr] [--timeout=SECS] <mode> <file_path> [additional_files...]",
            file=sys.stderr,
        )
    ...
    try:
        ...
        print(f"Error extracting with pypdfium2: {e}", file=sys.stderr)
        sys.exit(1)
